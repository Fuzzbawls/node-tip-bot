var logger = require('winston'),
	TimeBomb = require('../games/timebomb'),
...
module.exports = function (coin, ircClient, settings) {
	...
	timebomb.updatePotsBalances(function(err, balances) {
		if(err) {
			logger.error('Could not connect to %s RPC API! ', settings.coin.full_name, err);
			return;
		}

		logger.info('Timebomb: Pot balance is %d %s', balances.pot, settings.coin.short_name);
		logger.info('Timebomb: Rain pot balance is %d %s', balances.rainPot, settings.coin.short_name);
	});
	...
	coin.getAccountAddress(timebomb.potAccountName, function(err, address) {
		...
		logger.info('Timebomb: Pot address is %s', address);
	});
	...
	coin.getAccountAddress(timebomb.rainPotAccountName, function(err, address) {
		...
		logger.info('Timebomb: Rain Pot address is %s', address);
	});
			...
			switch(command) {
				...
				case 'cut':
					if (timebomb.canCutWire(from)) {
						var match = message.match(/^.?cut(?:wire)? (\S+)$/);
						if (match == null) {
				...
				case 'pot':
					timebomb.updatePotsBalances(function(err, balances) {
						...
						ircClient.say(channel, timebomb.i18n.potinfo.expand({rain_pot_balance: balances.rainPot, pot_balance: balances.pot, max_rain_fund: timebomb.settings.maxPot}));
					});
					break;
			}
        },
        commands: ['bomb', 'again', 'stop', 'cutwire', 'cut', 'pot']
    };
