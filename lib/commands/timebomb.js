...
module.exports = function (coin, ircClient, settings) {
	...
	timebomb.updatePotsBalances(function(err, balances) {
		...
		logger.info('Pot balance is %d %s', balances.pot, settings.coin.short_name);
		logger.info('Rain pot balance is %d %s', balances.rainPot, settings.coin.short_name);
	});
			...
			switch(command) {
				case 'bomb':
					...
					if (timebomb.canStartGame(bet)) {
						...
						ircClient.say(channel, timebomb.i18n.nofundsinpottostart.expand({}));
				...
				case 'cut':
					if (timebomb.canCutWire(from)) {
						...
						if (match == null) {
							ircClient.say(channel, timebomb.i18n.cutwireusage.expand({}));
							return;
						}
						var color = match[1];
