...
module.exports = function (coin, ircClient, settings) {
	...
	.on('boom', function(lostAmount) {
		ircClient.say(this.channel, this.i18n.boom.expand({lost_amount: lostAmount, pot_balance: this.getPotBalance(), rain_pot_balance: this.getRainPotBalance(), max_rain_fund: this.settings.maxPot}));
	})
	.on('log', function(msg) {
		logger.info(msg);
	})
        ...
        run: function (from, channel, message) {
			...
			switch(command) {
				case 'bomb':
					...
					if(match == null) {
						ircClient.say(this.channel, this.i18n.usage.expand({min_bet: this.settings.minBet, max_bet: this.settings.maxBet}));
						return;
					}
					...
					if (timebomb.canStartGame(bet)) {
						if (timebomb.gameInProgress) {
							var timeLeft = timebomb.getTimeLeft();
							if (timeLeft>0) {
								ircClient.say(channel, timebomb.i18n.bombalreadyactivated.expand({player: from, timeLeft: timeLeft}));
							} else {
								ircClient.say(channel, timebomb.i18n.gameinprogress.expand({player: from}));
							}
						} else {
