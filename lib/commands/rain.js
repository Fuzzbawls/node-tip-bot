...
module.exports = function (coin, ircClient, settings) {
    return function rain(from, channel, message) {
        var last_active = require('../../tipbot').last_active,
            ...
            blacklist = [from, 'ChanServ', settings.login.nickname];

        // lock
        if (locks.hasOwnProperty(from.toLowerCase()) && locks[from.toLowerCase()]) {
            logger.warn('Lock on ' + from + ' detected. Aborting.');
            return;
        }
            ...
            if(balance >= amount) {
                ircClient.getNames(channel, function(names) {
                  ...
                  if(settings.commands.rain.rain_on_last_active) {
                    ...
                    // remove blacklisted nicknames from tipping list
                    var blacklistedNames = blacklist.slice();
                    var name=blacklistedNames.shift();
                    while(name) {
                        // remove tipper from the list
                        names.splice(names.indexOf(name), 1);
                        name=blacklistedNames.shift();
                    }
