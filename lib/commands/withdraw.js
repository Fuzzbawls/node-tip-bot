...
module.exports = function (coin, ircClient, settings) {
    return function withdraw(from, channel, message) {
        ...
        if(match == null) {
            ircClient.say(channel, 'Usage: ' + settings.commandprefix + 'withdraw <' + settings.coin.full_name + ' address> [amount]');
            return;
        }
        var address = match[1];
        ...
        coin.validateAddress(address, function(err, reply) {
            if(err) {
                logger.error('Error in ' + settings.commandprefix + 'withdraw command', err);
