    ...
    switch(command) {
       case 'rainactive':
        ...
        if(match == null || !match[3]) {
          client.say(channel, 'Usage: !rainactive <amount> [active X minutes ago]');
          return;
        }
         ...
         if(balance >= amount) {
            client.getNames(channel, function(names) {
              ...
              if(amount / names.length < settings.coin.min_rain) {
                client.say(channel, settings.messages.rain_too_small.expand({from: from, amount: amount, min_rain: settings.coin.min_rain * names.length}));
                return;
              }
              ...
              client.say(channel, settings.messages.rain.expand({name: from, amount: amount / names.length, list: names.join(', ')}));
            });
          } else {
